<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="/css/app.css">
    <!-- <link rel="stylesheet" href="/css/normalize.css"> -->
    <link href="https://unpkg.com/tailwindcss@^2/dist/tailwind.min.css" rel="stylesheet">
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Open+Sans:wght@400;600;700&display=swap" rel="stylesheet"> 
    <title>CSS for Backend Devs - #2</title>
</head>
<body class="flex items-center justify-center">
    <div class="card flex rounded">

        <div class="card-left flex flex-col justify-between p-6 items-center text-center text-white rounded">

            <a href="#" class="card-skill-button no-underline text-xs rounded-full py-1 px-6 text-inherit">PHP</a>

            <img src="https://laracasts.s3.amazonaws.com/series/thumbnails/10-techniques-for-cleaner-code.png?v=8" width="98" height="98">

            <span class="card-difficulty text-xs">
                Intermediate Difficulty
            </span>
        </div>

        <div class="card-right flex flex-col p-6 text-gray-800">
            <h3 class="card-title font-light text-3xl mb-3 text-black">
                <a href="#" class="no-underline text-inherit">Testing Jargon</a>
            </h3>

            <p class="card-excerpt flex-1">
                There's no two ways about it: terminology in the testing world is incredibly overwhelming. Let's fix that! Bit by bit, we'll break all of these confusing concepts down as best we can.
            </p>

            <div class="card-meta flex text-sm">
                <div class="flex items-center mr-4">
                    <img src="https://image.flaticon.com/icons/svg/864/864685.svg"  width="15" height="15" class="mr-2">
                    5 Lessons 
                </div>
                
                <div class="flex items-center">
                    <img src="https://image.flaticon.com/icons/svg/2088/2088617.svg"  width="15" height="15" class="mr-2">
                    53:10 min
                </div>
            </div>
        </div>
    </div>
</body>
</html>